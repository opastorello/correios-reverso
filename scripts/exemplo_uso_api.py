...
def testar_conexao() -> bool:
    """Testa conexao e login com Correios."""
    print_section("TESTAR CONEXO")
    ...
    try:
        with CorreiosClient.from_env() as client:
            ...
            for c in cartoes.itens:
                ...
                print(f"  Cartao: {c.numeroCartao} - Contrato: {contrato}")
            return True
...
def listar_servicos(logistica_reversa: bool = False) -> None:
    ...
    titulo = "LISTAR SERVIOS (LOGISTICA REVERSA)" if logistica_reversa else "LISTAR SERVIOS"
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Total: {len(servicos)} servicos\n")
        for s in servicos:
...
def listar_postagens(limite: int = 5, logistica_reversa: bool = False) -> None:
    """Lista pre-postagens registradas."""
    titulo = "LISTAR PR-POSTAGENS (LR)" if logistica_reversa else "LISTAR PR-POSTAGENS"
    print_section(titulo)
...
def listar_destinatarios() -> None:
    """Lista destinatarios cadastrados."""
    print_section("LISTAR DESTINATARIOS")
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Total: {total} destinatarios\n")
...
def criar_e_cancelar_postagem(tipo: str = "normal") -> None:
    """Cria uma pre-postagem de teste e cancela em seguida."""
    titulo = f"CRIAR E CANCELAR PR-POSTAGEM ({tipo.upper()})"
    print_section(titulo)
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Pre-postagens antes: {antes}")
        ...
        if item_criado:
            ...
            print(f"  Codigo: {item_criado.codigoObjeto}")
            ...
            print(f"\nPre-postagens final: {final} (esperado: {antes})")
        else:
            print(" Item criado nao encontrado na lista")
...
def testar_api_rest(base_url: str = "http://localhost:8000", token: str = "") -> None:
    ...
    # Postagens
    print("\n--- Postagens ---")
    postagens = get("/postagem[OK]pagina=0")
    print(f"Total: {postagens.get('page', {}).get('count', 0)} pre-postagens")

    # Destinatarios
    print("\n--- Destinatarios ---")
    dests = get("/destinatarios")
    print(f"Total: {dests.get('page', {}).get('count', 0)} destinatarios")
...
def main():
    ...
    parser.add_argument("--listar", action="store_true", help="Listar pre-postagens")
    parser.add_argument("--servicos", action="store_true", help="Listar servicos")
    parser.add_argument("--destinatarios", action="store_true", help="Listar destinatarios")
    ...
    parser.add_argument("--token", default="", help="Token de autenticacao")
    ...
    try:
        if run_all or not args.api:
            ...
            if not testar_conexao():
                print("\n Falha na conexao. Verifique o arquivo .env")
                return 1
