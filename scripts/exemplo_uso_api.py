...
def testar_conexao() -> bool:
    """Testa conexão e login com Correios."""
    print_section("TESTAR CONEXÃO")
    ...
    try:
        with CorreiosClient.from_env() as client:
            ...
            for c in cartoes.itens:
                ...
                print(f"  Cartão: {c.numeroCartao} - Contrato: {contrato}")
            return True
...
def listar_servicos(logistica_reversa: bool = False) -> None:
    ...
    titulo = "LISTAR SERVIÇOS (LOGÍSTICA REVERSA)" if logistica_reversa else "LISTAR SERVIÇOS"
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Total: {len(servicos)} serviços\n")
        for s in servicos:
...
def listar_postagens(limite: int = 5, logistica_reversa: bool = False) -> None:
    """Lista pré-postagens registradas."""
    titulo = "LISTAR PRÉ-POSTAGENS (LR)" if logistica_reversa else "LISTAR PRÉ-POSTAGENS"
    print_section(titulo)
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Total: {result.page.count} pré-postagens\n")
...
def listar_destinatarios() -> None:
    ...
    print_section("LISTAR DESTINATÁRIOS")
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Total: {total} destinatários\n")
...
def criar_e_cancelar_postagem(tipo: str = "normal") -> None:
    """Cria uma pré-postagem de teste e cancela em seguida."""
    titulo = f"CRIAR E CANCELAR PRÉ-POSTAGEM ({tipo.upper()})"
    print_section(titulo)
    ...
    with CorreiosClient.from_env() as client:
        ...
        print(f"Pré-postagens antes: {antes}")
        ...
        print(f"\nPré-postagens depois: {depois} (esperado: {antes + 1})")
        ...
        if item_criado:
            ...
            print(f"  Código: {item_criado.codigoObjeto}")
            ...
            print(f"\nPré-postagens final: {final} (esperado: {antes})")
        else:
            print("⚠ Item criado não encontrado na lista")
...
def testar_api_rest(base_url: str = "http://localhost:8000", token: str = "") -> None:
    ...
    print(f"Total: {postagens.get('page', {}).get('count', 0)} pré-postagens")

    # Destinatários
    print("\n--- Destinatários ---")
    dests = get("/destinatarios")
    print(f"Total: {dests.get('page', {}).get('count', 0)} destinatários")
...
def main():
    ...
    parser.add_argument("--listar", action="store_true", help="Listar pré-postagens")
    parser.add_argument("--servicos", action="store_true", help="Listar serviços")
    parser.add_argument("--destinatarios", action="store_true", help="Listar destinatários")
    ...
    try:
        if run_all or not args.api:
            ...
            if not testar_conexao():
                print("\n✗ Falha na conexão. Verifique o arquivo .env")
                return 1
